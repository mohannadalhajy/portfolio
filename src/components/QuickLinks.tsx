...
const links = [
  {
    ...
    copyValue: "https://github.com/mohannadalhajy",
    icon: (
  ...
  },
  {
    ...
    copyValue: "https://linkedin.com/in/mohannad-alhajy",
    icon: (
      <svg viewBox="0 0 24 24" className="h-5 w-5 fill-current">
  ...
  },
  {
    ...
    copyValue: "mohannad.alhajy@gmail.com",
        ...
        {links.map((link) => (
          ...
          >
            ...
            <div className="relative z-10 flex shrink-0 items-center gap-2">
              {link.copyValue && (
                <CopyButton value={link.copyValue} label={`Copy ${link.title.toLowerCase()}`} />
              )}
              <ArrowUpRight className="pointer-events-none h-5 w-5 text-zinc-600 transition-all group-hover:-translate-y-0.5 group-hover:translate-x-0.5 group-hover:text-zinc-100" />
            </div>
          </div>
        ))}
      </div>
